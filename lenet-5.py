...
parser.add_argument('--num_epochs', type=int, default=300, help='Number of epochs to train for')#46               #
...
parser.add_argument('--resize_height', type=int, default=32, help='Height of cropped input image to network')#VGG19 331 #224
parser.add_argument('--resize_width', type=int, default=32, help='Width of cropped input image to network')
parser.add_argument('--image_mod', type=str, default="grayscale", help='rgb or grayscale')
...
parser.add_argument('--model', type=str, default="LetNet-5", help='Your pre-trained classification model of choice')#VGG16 0.8379# #AlexNet_0.5 0.7489#  #LeNet-5 196  0.5961#
...
model = Sequential()
model.add(Conv2D(6, (5, 5), strides=(1, 1),input_shape = (32,32,1), padding='same', activation='relu',
                 kernel_initializer='uniform'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Conv2D(16, (5, 5), strides=(1, 1), padding='same', activation='relu', kernel_initializer='uniform'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Flatten())
...
if args.mode == "train":
    ...
    train_generator = train_datagen.flow_from_directory(TRAIN_DIR,color_mode = IMAGEMOD, target_size=(HEIGHT, WIDTH), batch_size=BATCH_SIZE)

    validation_generator = val_datagen.flow_from_directory(VAL_DIR,color_mode = IMAGEMOD, target_size=(HEIGHT, WIDTH), batch_size=BATCH_SIZE)
